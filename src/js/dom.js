...
export const createOrderMessage = (id) => {
  ...
  orderMessageText.textContent = `Ваш заказ оформлен, номер заказа ${id}. Вы можете его забрать завтра после 12:00.`;
...
const createProductCard = ({ id, photoUrl, name, price }) => {
  ...
  productCard.innerHTML = `
    <article class="store__product product">
      ...
      <button class="product__btn-add-cart" data-id="${id}"  aria-label="Заказать">Заказать</button>
    </article>
  `;
  ...
  products.forEach(({ id, photoUrl, name, price }) => {
    ...
    listItem.innerHTML = `
      ...
      <div class="modal__cart-item-count">
        <button class="modal__btn modal__minus" data-id="${id}" aria-label="Удалить 1 товар из корзины">-</button>
        <span class="modal__count">${cartItem.count}</span>
        <button class="modal__btn modal__plus" data-id="${id}" aria-label="Добавить 1 товар в корзину">+</button>
      </div>
